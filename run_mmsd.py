...
parser.add_argument('--deepspeed_config', type=str, required='-deepspeed' in sys.argv, help='DeepSpeed config file')
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, prompt):
        ...
        # print(att_mask.shape)
        # print(pos_ids.shape)
        dec_out = self.llm_model(inputs_embeds=llm_enc_out.bfloat16(),attention_mask=att_mask,position_ids=pos_ids)
...
def open_json(path):
    f=open(path,"r")
    js=json.load(f)
    re={'Imagepath':[],'Text':[],'Multi_label':[]}
    for i in js:
        re['Imagepath'].append("./MMSD2.0/data/dataset_image/"+str(i["image_id"])+".jpg")
        re["Text"].append(i["text"])
        re['Multi_label'].append(i["label"])
    return re

    
df_train = open_json("./MMSD2.0/data/text_json_clean/train.json")
df_test = open_json("./MMSD2.0/data/text_json_clean/test.json")
df_val = open_json("./MMSD2.0/data/text_json_clean/valid.json")



train_dataset = MultiDataset(df_train['Imagepath'],df_train['Text'],df_train['Multi_label'], resize=512, train_mode=True)
test_dataset = MultiDataset(df_test['Imagepath'],df_test['Text'],df_test['Multi_label'], resize=512, train_mode=False)
val_dataset = MultiDataset(df_val['Imagepath'],df_val['Text'],df_val['Multi_label'], resize=512, train_mode=False)
...
scaler = GradScaler()
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim,T_max=2*len(train_loader), eta_min=1e-8)
...
train_loader,val_loader, test_loader,  model, model_optim, scheduler = train_loader,val_loader, test_loader, model, optim, scheduler
